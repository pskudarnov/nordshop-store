...
export async function generateMetadata({
  ...
  if (!setup) {
    return {
      ...
      description:
        locale === "ru" ? "Запрошенный сетап недоступен." : "Requested setup is unavailable.",
    };
  }
  ...
  return (
    <section className="py-8 sm:py-10">
      ...
      <header className="relative overflow-hidden rounded-3xl border border-[var(--border)] bg-[linear-gradient(140deg,color-mix(in_srgb,var(--accent)_15%,transparent),color-mix(in_srgb,var(--card)_85%,var(--muted)))] p-5 sm:p-8">
        <div
          className="absolute -right-16 -top-12 h-44 w-44 rounded-full bg-[color-mix(in_srgb,var(--accent)_22%,transparent)] blur-3xl"
          aria-hidden="true"
        />
        <div className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-base text-[var(--muted-foreground)] sm:text-lg">
              {content.subtitle}
            </p>
            <div className="mt-4 flex flex-wrap gap-2">
              {content.tags.map((tag) => (
                <span
                  key={tag}
                  className="rounded-full border border-[var(--border)] bg-[color-mix(in_srgb,var(--card)_85%,var(--muted))] px-3 py-1 text-xs font-medium text-[var(--foreground)]"
                >
                  {tag}
                </span>
              ))}
      ...
      <section id="setup-breakdown" className="mt-9">
        <h2 className="text-2xl font-semibold tracking-tight text-[var(--foreground)]">
          {labels.setupBreakdown}
        </h2>
        <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {gear.map((item) => (
            <article
              key={`${item.role}-${item.product.slug}`}
              className="group flex h-full flex-col overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-[var(--shadow-soft)] transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[var(--shadow-soft-lg)]"
            >
              <div className="relative aspect-[4/3] overflow-hidden border-b border-[var(--border)] bg-[var(--muted)]">
                <ProductImage
                  src={item.product.image}
                  alt={`${item.product.name} ${item.role}`}
                  fill
              ...
              <div className="flex flex-1 flex-col p-4">
                ...
                <Link
                  ...
                  className="mt-4 inline-flex items-center gap-1.5 text-sm font-medium text-[var(--accent)] transition hover:gap-2"
                >
                  {labels.viewProduct}
                  <ChevronRight className="h-4 w-4" />
                </Link>
      ...
      <section className="mt-9">
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-4">
          {content.gallery.map((image) => (
            <figure
              key={image.src}
              className="group relative aspect-[4/3] overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--muted)]"
            >
              <ProductImage
                src={image.src}
                alt={image.alt}
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-3">
          {relatedSetups.map((item) => (
            <Link
              ...
              className="group overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-[var(--shadow-soft)] transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[var(--shadow-soft-lg)]"
            >
              <div className="relative aspect-[4/3] border-b border-[var(--border)] bg-[var(--muted)]">
                <ProductImage
                  src={item.content.heroImage}
                  alt={item.content.heroAlt}
                  fill
                  className="object-cover transition-transform duration-500 group-hover:scale-[1.04]"
                  sizes="(max-width: 768px) 100vw, 33vw"
                  loading="lazy"
      ...
      <section className="mt-10 overflow-hidden rounded-3xl border border-[var(--border)] bg-[linear-gradient(145deg,color-mix(in_srgb,var(--muted)_72%,var(--card)),color-mix(in_srgb,var(--accent)_10%,transparent))] px-5 py-8 sm:px-8">
        ...
        <Link
          href="/products"
          className="mt-6 inline-flex items-center gap-2 rounded-xl bg-[var(--primary)] px-4 py-2.5 text-sm font-medium text-[var(--primary-foreground)] shadow-[var(--shadow-soft)] transition-all duration-200 hover:brightness-110 hover:shadow-[var(--shadow-soft-lg)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ring)] active:brightness-95"
        >
          {labels.ctaButton}
          <ArrowUpRight className="h-4 w-4" />
        </Link>
