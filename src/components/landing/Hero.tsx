...
export function Hero() {
  ...
  return (
    <Section className="grid gap-10 pb-10 pt-11 md:items-start md:grid-cols-[1.1fr_0.9fr] md:gap-11 md:pb-14 md:pt-15">
      <div>
        <p className="inline-flex rounded-full border border-[var(--border)] bg-[color-mix(in_srgb,var(--accent)_16%,var(--muted))] px-3 py-1 text-[11px] font-medium uppercase tracking-[0.12em] text-[var(--foreground)]">
          {t("badge")}
        </p>
        <h1 className="mt-5 max-w-3xl text-4xl font-semibold tracking-[-0.02em] text-[var(--foreground)] md:text-6xl md:leading-[1.04]">
          {t("title")}
        </h1>
        <p className="mt-5 max-w-2xl text-base text-[var(--muted-foreground)] md:text-lg md:leading-[1.65]">
          {t("subtitle")}
        </p>
        ...
        <div className="mt-7 flex flex-wrap gap-2 text-xs text-[var(--foreground)]">
          <span className="rounded-full border border-[var(--border)] bg-[var(--card)] px-3 py-1">
            {t("trust1", { value: formatCurrency(freeShippingThreshold, locale) })}
          </span>
          <span className="rounded-full border border-[var(--border)] bg-[var(--card)] px-3 py-1">
            {t("trust2")}
          </span>
      ...
      <aside className="premium-panel relative self-start overflow-hidden rounded-3xl p-6 md:p-8">
        <div
          className="absolute -right-10 -top-10 h-40 w-40 rounded-full bg-[color-mix(in_srgb,var(--accent)_28%,transparent)] blur-3xl"
          aria-hidden
        />
        ...
        <div className="mt-5 grid grid-cols-[1.35fr_1fr] gap-2.5">
          <div className="relative aspect-[4/3] overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--muted)] p-2.5">
            <ProductImage
              src="/images/products/fjord-mech-keyboard-main.webp"
              alt={t("bundleAlt1")}
              fill
