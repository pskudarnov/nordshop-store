...
function CartButton({ count }: { count: number }) {
  return (
    <span className="relative inline-flex rounded-xl border border-[var(--border)] bg-[var(--card)] p-2.5 text-[var(--foreground)] transition hover:bg-[var(--muted)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ring)]">
      ...
      {count > 0 ? (
        <span className="absolute -right-1 -top-1 inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-[var(--primary)] px-1 text-[10px] font-semibold leading-none text-[var(--primary-foreground)] ring-2 ring-[var(--background-elevated)]">
          {count}
        </span>
      ) : null}
    </span>
...
export function Header() {
  ...
  const handleAnchorClick = (
    ...
    const localizedHash =
      href === "/#products" ? `${localizedRoot}#products` : href.replace("/#", `${localizedRoot}#`);
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[var(--border)] bg-[color-mix(in_srgb,var(--glass)_92%,transparent)] backdrop-blur-xl">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-2 px-3 py-2.5 sm:gap-3 sm:px-6">
        ...
        <nav
          className="hidden min-w-0 flex-1 items-center justify-center gap-5 text-sm md:flex lg:gap-7"
          aria-label={t("a11y.mainNav")}
        >
          {links.map((link) => (
            <Link
              key={link.href}
              className="whitespace-nowrap text-[var(--muted-foreground)] transition hover:text-[color:#9a7b3a]"
              href={link.href}
          ...
          <Popover.Root open={desktopCartOpen} onOpenChange={setDesktopCartOpen}>
            ...
            <Popover.Portal>
              <Popover.Content
                ...
                sideOffset={12}
                collisionPadding={16}
                className="z-[100] w-[392px] overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-[var(--shadow-soft-lg)]"
              >
            ...
            <div className="flex items-center gap-1.5 rounded-xl border border-[var(--border)] bg-[color-mix(in_srgb,var(--card)_92%,var(--muted))] p-1 shadow-[var(--shadow-soft)]">
              ...
              >
                ...
                <span className="inline-flex items-center gap-1.5">
                  <span className="inline-flex min-w-5 items-center justify-center rounded-full bg-[color-mix(in_srgb,var(--accent)_18%,var(--muted))] px-1.5 py-0.5 text-[10px] font-semibold text-[var(--foreground)]">
                    {count}
                  </span>
