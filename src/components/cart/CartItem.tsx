    ...
    <article className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-4 text-[var(--card-foreground)] shadow-[var(--shadow-soft)] transition duration-200 hover:translate-y-[-1px]">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-[var(--muted-foreground)]">{t('unitPrice')}: <span className="font-medium text-[var(--foreground)]"><Price value={product.price} /></span></p>
        </div>
        <div className="text-right">
          <p className="text-xs uppercase tracking-[0.14em] text-[var(--muted-foreground)]">{t('lineTotal')}</p>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-[var(--border)] pt-3">
        ...
        <Button variant="ghost" onClick={onRemove} aria-label={t('removeAria', {name: product.name})} className="gap-1.5">
